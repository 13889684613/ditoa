<?php
	...
	$where .= 'or (receiverRole=1 and receiverOffice='.$common_officeId.')';
	$where .= ' or (receiverRole=2 and receiverGroup='.$common_groupId.')';
	...
	//未读消息数量
	$noRead = $db->count(PRFIX.'message',$where);
	...
	$where = 'where (toRole=0 or (toRole=1 and find_in_set('.$common_officeId.',toOffice) or (toRole=2 and find_in_set('.$common_groupId.',toGroup))';
