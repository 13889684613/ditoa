<?php
	...
	//array('0'=>姓名,'1'=>所属公司,'2'=>所属部门,'3'=>所属工作组,'4'=>审批角色)
	function getStaffInfo($staffId){
		...
		if($staffId!=''&&$staffId!=0&&is_numeric($staffId)){
			...
			//拉取员工核心数据
			global $db;
			$S = $db->get_one(PRFIX.'staff','where staffId='.$staffId.'','staffName,companyId,officeId,groupId,checkRoleId');
			if($S){
				$staffInfo[0] = $S['staffName'];
				$staffInfo[1] = $S['companyId'];
				$staffInfo[2] = $S['officeId'];
				$staffInfo[3] = $S['groupId'];
				$staffInfo[4] = $S['checkRoleId'];
			}
