...
{ 
    ...
    public function insert($table,$dataArray,$debug=0) {  
    ...
    public function get_one($table,$where="",$col="*",$debug=0) {  
    ...
    public function get_all($table,$where="",$col="*",$debug=0) {  
        $sql="SELECT $col FROM $table $where"; 
        if ($debug == 1){
        	echo $sql;exit;
        }
    ...
    public function get_some($table,$start,$length,$order="",$where="",$col="*",$debug=0) {  
		$result_array = array();  
		switch($this->db_type) { 
			case "mysql": 
    ...
    public function update($talbe, $dataArray, $where,$debug=0) {  
       	while(list($key,$val) = each($dataArray)) { 
			if(is_string($val)) 
				$value .= "$key = '$val',";  
			else 
