<?php
	...
	foreach ($data as $key => $value) {
		...
		if($checkStatus == 2||$checkStatus == 3||$checkStatus == 4){
			$check = $db->get_one(PRFIX.'quitapply_check','where quitApplyId='.$data[$key]['quitApplyId'].' and checkLevel='.$data[$key]['curCheckLevel'].'','remark,checkUsr');
			if($check['remark']!=''){
